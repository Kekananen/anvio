...
"""
    ...
    run:
        ...
        item_order_name = (
            "%s:unknown:unknown" % filesnpaths.get_name_from_file_path(input.tree)
        )
        p_meta["default_item_order"] = item_order_name
        p_meta["available_item_orders"].append(item_order_name)
        p_meta["item_orders"][item_order_name] = {
            "type": "newick",
            "data": newick_tree_text,
        }
        p_meta["default_view"] = "single"
        ...
        shell(
            "anvi-import-items-order -p {params.tree_profileDB} -i {input.tree} --name {wildcards.group}_tree >> {log} 2>&1"
        )
        shell(
            "anvi-import-misc-data -p {params.tree_profileDB} --target-data-table items {params.misc_data} --just-do-it >> {log} 2>&1"
        )
        shell(
            "anvi-import-state -p {params.tree_profileDB} -s {input.state} -n default >> {log} 2>&1"
        )
        if os.path.isfile(params.tax_data_final):
            shell(
                "anvi-import-misc-data -p {params.tree_profileDB} --target-data-table items {params.tax_data_final} --just-do-it >> {log} 2>&1"
            )
